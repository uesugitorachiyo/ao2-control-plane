...
def write_release_view(path, asset_names):
    path.write_text(
        json.dumps(
            {
                "tagName": "v0.1.17",
                "name": "ao2-control-plane v0.1.17 stable",
                "isDraft": False,
                "isPrerelease": False,
                ...
                "url": "https://github.com/uesugitorachiyo/ao2-control-plane/releases/tag/v0.1.17",
            },
            indent=2,
            sort_keys=True,
        )
...
def write_release_notes(path, note_checksums):
    ...
    labels = {
        "ao2-control-plane-0.1.17-macos-aarch64.tar.gz": "macOS aarch64",
        "ao2-control-plane-0.1.17-linux-x86_64.tar.gz": "Linux x86_64",
        "ao2-control-plane-0.1.17-windows-x86_64.tar.gz": "Windows x86_64",
    }
...
def test_release_asset_parity_audit_passes_complete_three_os_stable_release(tmp_path):
    ...
    assert summary["release_tag"] == "v0.1.17"
...
def test_release_asset_parity_audit_reports_release_notes_checksum_drift(tmp_path):
    ...
    note_checksums["ao2-control-plane-0.1.17-macos-aarch64.tar.gz"] = "f" * 64
    ...
    assert summary["release_notes_checksum_drift"] == [
        {
            "asset": "ao2-control-plane-0.1.17-macos-aarch64.tar.gz",
    ...
    assert summary["gaps"][-1] == {
        ...
        "assets": ["ao2-control-plane-0.1.17-macos-aarch64.tar.gz"],
    }
