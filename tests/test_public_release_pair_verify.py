...
def test_public_release_pair_verify_defaults_follow_release_train_manifest():
    ...
    assert manifest["stable"]["ao2"] == {"tag": "v0.5.11", "version": "0.5.11"}
...
def run_pair_verify(
    ...
    *,
    ...
    write_release_view(ao2_view, "uesugitorachiyo/ao2", "v0.5.11", "AO2 v0.5.11", ao2_release_assets)
...
def test_public_release_pair_verify_passes_complete_ao2_and_control_plane_releases(tmp_path):
    ...
    assert summary["ao2"]["release_tag"] == "v0.5.11"
...
def test_public_release_pair_verify_reports_missing_ao2_promotion_plan_gap(tmp_path):
    assets = [asset for asset in ao2_assets() if asset != "promotion-plan.json"]
...
def test_release_train_manifest_rejects_tag_version_mismatch(tmp_path):
    ...
    manifest["stable"]["ao2"] = {"tag": "v0.5.9", "version": "0.5.6"}
