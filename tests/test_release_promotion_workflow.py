...
def test_release_promotion_publish_step_is_explicitly_guarded():
    ...
    for needle in [
        ...
        'reviewer_rule.get("prevent_self_review") is not True',
        'reviewer_logins != {owner}',
        "self-review may be enabled only for the sole repository-owner reviewer",
        "release_environment_review_mode=single_owner_explicit",
...
def test_public_download_and_authentication_docs_match_current_contract():
    ...
    assert "v0.1.19" in public_download
    assert "ao2-control-plane-0.1.19-macos-aarch64.tar.gz" in public_download
