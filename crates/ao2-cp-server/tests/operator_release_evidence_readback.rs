...
async fn operator_release_evidence_json_and_dashboard_are_read_only_observer_surfaces() {
    ...
    let dual_public = body["operator_release_evidence"]["checks"]
        ...
        .unwrap()
        .iter()
        .find(|check| check["artifact"] == "ao2-dual-public-release-smoke")
        .expect("dual public release smoke check is present");
    assert_eq!(
        dual_public["schema_version"],
        "ao2.dual-public-release-smoke.v1"
    );
    assert_eq!(
        dual_public["task_board_readback_schema"],
        "ao2.cp-ai-task-board-readback.v1"
    );
    assert_eq!(
        dual_public["task_board_dashboard_schema"],
    ...
    assert!(html.contains("ao2-dual-public-release-smoke"));
