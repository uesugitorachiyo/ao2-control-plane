...
pwsh -File Verify-ReleaseSupportBundle.ps1 -Checksums SHA256SUMS -Path {filename}</code></pre></section><p><a href=\"{download_url}\">Download JSON</a> · <a href=\"{checksums_url}\">SHA256SUMS</a> · <a href=\"{manifest_json}\">Manifest JSON</a> · <a href=\"{verify_html}\">Verification HTML</a> · <a href=\"{verify_json}\">Verification JSON</a></p></main></body></html>",
        status_class = if json_str(&manifest, "status") == Some("passed") { "ok" } else { "bad" },
        status = escape_html(json_str(&manifest, "status").unwrap_or("unknown")),
        filename = escape_html(json_str(&manifest, "filename").unwrap_or("release-support-bundle.json")),
        ...
        download_url = escape_html(json_str(&links, "release_support_bundle_download").unwrap_or("#")),
        checksums_url = escape_html(json_str(&links, "release_support_bundle_checksums").unwrap_or("#")),
        manifest_json = escape_html(json_str(&links, "release_support_bundle_manifest_json").unwrap_or("#")),
        verify_html = escape_html(json_str(&links, "release_support_bundle_verify_html").unwrap_or("#")),
        verify_json = escape_html(json_str(&links, "release_support_bundle_verify_json").unwrap_or("#")),
    );
    Ok((
        StatusCode::OK,
        [(header::CONTENT_TYPE, "text/html; charset=utf-8")],
...
pub async fn release_support_verifier_handoff(
    ...
    let html = format!(
        ...
        approves = escape_html(&handoff.get("control_plane_approves_release").map(json_scalar).unwrap_or_else(|| "missing".to_string())),
        mutates = escape_html(&handoff.get("mutates_ao_artifacts").map(json_scalar).unwrap_or_else(|| "missing".to_string())),
        check_rows = check_rows,
        handoff_json = escape_html(json_str(&links, "release_support_verifier_handoff_json").unwrap_or("#")),
        verify_json = escape_html(json_str(&links, "release_support_bundle_verify_json").unwrap_or("#")),
...
pub async fn release_support_bundle_verification(
    ...
    let check_rows = checks
        ...
        .map(|check| {
            ...
            format!(
                ...
                declared = escape_html(json_str(check, "declared_schema_version").unwrap_or("missing")),
                embedded = escape_html(json_str(check, "embedded_schema_version").unwrap_or("missing")),
                digest = escape_html(json_str(check, "recomputed_sha256").unwrap_or("missing")),
                class = class,
                status = escape_html(status),
            )
    ...
    let html = format!(
        ...
        mutates = escape_html(&trust.get("mutates_ao_artifacts").map(json_scalar).unwrap_or_else(|| "missing".to_string())),
        check_rows = check_rows,
...
fn normalize_release_support_storage_surface(
...
fn release_support_bundle_filename(bundle: &serde_json::Value) -> String {
    let version = bundle
        .get("release")
        .and_then(|release| json_str(release, "version"))
        .unwrap_or("unknown");
...
fn release_support_bundle_manifest_value(
...
fn release_support_verifier_handoff_value(
    bundle: &serde_json::Value,
    keep_latest: usize,
) -> Result<serde_json::Value, AppError> {
    let verification = release_support_bundle_verification_value(bundle)?;
...
fn release_support_bundle_checksums_text(bundle: &serde_json::Value) -> Result<String, AppError> {
    let verification = release_support_bundle_verification_value(bundle)?;
    let filename = release_support_bundle_filename(bundle);
    let bundle_sha256 = json_str(&verification, "bundle_sha256").unwrap_or("missing");
...
fn release_support_bundle_verification_value(
    bundle: &serde_json::Value,
) -> Result<serde_json::Value, AppError> {
    let bundle_sha256 = support_bundle_surface_sha256(bundle)?;
    let surfaces = bundle
...
fn support_bundle_surface_by_id<'a>(
...
fn support_bundle_surface_path_by_id(id: &str) -> Option<&'static str> {
    match id {
...
fn release_readiness_gate_with_detail(
    id: &str,
    label: &str,
    observed: &str,
    expected: &str,
...
fn same_candidate_republish_action() -> &'static str {
...
fn provider_acceptance_readiness(entry: &serde_json::Value) -> &'static str {
...
async fn get_release_evaluator_decision_signature_by_sha(
    state: &AppState,
    sha: &str,
) -> Result<Response, AppError> {
    validate_sha(sha)?;
...
async fn release_evaluator_decision_signature_value(
    state: &AppState,
    sha: &str,
...
/// evaluator-closer's.
fn verify_release_evaluator_decision_signature(
    decision_raw: &str,
    signature: &serde_json::Value,
...
fn decode_release_evaluator_signature_hex(value: &str) -> Result<Vec<u8>, AppError> {
...
/// verified.
fn verify_release_evaluator_rsa_sha256_signature(
    decision_bytes: &[u8],
    signature_bytes: &[u8],
    public_key_pem: &str,
) -> Result<(), AppError> {
